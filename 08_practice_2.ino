...
static int distanceToPWM(float d_mm) {
  if (d_mm <= 0.0f || d_mm <= _DIST_MIN || d_mm >= _DIST_MAX) return 255; // off
// Active-Low: 밝기 1.0 -> PWM 0, 밝기 0.0 -> PWM 255
  int pwm = ((int)(255.0f * (200-d_mm) / (float)100 )); // 반올림
  if (pwm<0) return -pwm;
  return pwm;
}
void setup() {
